...
- **backend/** FastAPI. 두 흐름으로 나뉜다.
  ...
  - **LLM 선택**: `app/llm/`(registry+factory). `/models`로 목록 제공, `/search`의 `model` 파라미터로 선택.
    기본=HCX-30B-Text(hcx-agent-05, 사내 게이트웨이), 옵션=Qwen3.6-35B-A3B·HyperCLOVA X Think 32B·Claude Opus 4.8(anthropic).
    새 모델은 `app/llm/registry.py`의 MODELS에 1개 추가. 키는 `.env`.
