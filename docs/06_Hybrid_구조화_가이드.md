...
- `registry.py`에 `hcx30` 추가(label `HCX-30B-Text (hcx-agent-06)`), provider `openai_compat`.
- 사내 게이트웨이 `http://<host>:8000/v1/chat/completions`, thinking 모드.
- `ModelSpec.extra_body` 지원 → factory가 OpenAI SDK `extra_body`로 `chat_template_kwargs:{thinking:true}` 전달.
- `.env`: `HCX30_BASE_URL/HCX30_API_KEY/HCX30_MODEL`. **유일 운영 모델은 HCX-30B-Text**(대화·답변용).
  > 갱신(2026-07-21): 엔드포인트 포트 `11000`→`8000`, 모델 `hcx-agent-05`→`hcx-agent-06`로 변경.
